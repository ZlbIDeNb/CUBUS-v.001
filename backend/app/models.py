...
class RegistryClient(BaseModel):
    id: int
    name: str
    phones: list[RegistryPhone] = Field(default_factory=list)


class RegistryMeter(BaseModel):
    id: int
    device_kind: str = ""
    meter_type: str = ""
    serial_number: str = ""
...
class RegistryAddress(BaseModel):
    id: int
    address: str
    clients: list[RegistryClient] = Field(default_factory=list)
    meters: list[RegistryMeter] = Field(default_factory=list)
...
class DailyStatistics(BaseModel):
    ...
    rework: int = Field(ge=0)
    postponed: int = Field(ge=0)
...
class TodayStatistics(BaseModel):
    work_date: date
    initial_new: int = Field(ge=0)
    completed: int = Field(ge=0)
    rework: int = Field(ge=0)
    postponed: int = Field(ge=0)
    refusals: int = Field(ge=0)
    remaining_new: int = Field(ge=0)
    moved_to_other_date: int = Field(ge=0)
    transferred_to_other_employee: int = Field(ge=0)
    added_later: int = Field(ge=0)
    snapshot_at: str
...
class AdminEmployeeWorkspace(BaseModel):
    user: AdminUser
    report: PeriodReport
    warehouse: list[WarehouseItem] = Field(default_factory=list)
    applications: list[ApplicationSummary] = Field(default_factory=list)
    documents: list[DocumentationItem] = Field(default_factory=list)
    history: list[ActivityLogItem] = Field(default_factory=list)
