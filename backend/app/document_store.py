...
class DocumentStore:
    ...
    def replace(
        self, owner: str, document_id: int, command: DocumentationCreate
    ) -> DocumentationItem:
        with sqlite3.connect(self.database) as connection:
            exists = connection.execute(
                "SELECT 1 FROM documents WHERE owner=? AND id=?", (owner, document_id)
            ).fetchone()
        if exists is None:
            raise KeyError(document_id)
        replacement = self.add(owner, command)
        self.delete(owner, document_id)
        return replacement
