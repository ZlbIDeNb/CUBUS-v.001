...
def canonical_application_status(value: str) -> str | None:
    normalized = " ".join(value.casefold().replace("ё", "е").split())
    if normalized.startswith("нов"):
        return "Новая"
    if normalized.startswith("выполн"):
        return "Выполнено"
    if "доработ" in normalized:
        return "На Доработку"
    if normalized.startswith("отлож") or normalized == "в отложке":
        return "Отложено"
    if normalized.startswith("отказ") or normalized.startswith("отмен"):
        return "Отказ"
    return None
...
class ClientBaseClient:
    ...
    async def list_company_applications(
        ...
        applications = list(await asyncio.gather(*(self._summary(item) for item in rows)))
        return sorted(
            applications,
            key=lambda item: (
                not bool(item.delivery_time.strip()),
    ...
    async def list_company_tracking_applications(
        self, work_date: date
    ) -> list[ApplicationSummary]:
        date_field = self.fields["work_date"]
        day = work_date.isoformat()
        rows = await self._list_all(
            "data130",
            filter_expression=(
                f"and(eq(status,0),gte({date_field},'{day} 00:00:00'),"
                f"lte({date_field},'{day} 23:59:59'))"
            ),
        )
        return [self._tracking_summary(item) for item in rows]

    async def get_application_tracking_state(
        self, application_id: int
    ) -> ApplicationSummary:
        payload = await self._request("GET", f"data130/{application_id}")
        return self._tracking_summary(payload["data"])

    def _tracking_summary(self, item: dict) -> ApplicationSummary:
        attrs = item["attributes"]
        return ApplicationSummary(
            id=int(item["id"]),
            number=str(attrs.get(self.fields["number"], "") or ""),
            work_date=attrs.get(self.fields["work_date"]),
            status=str(attrs.get(self.fields["status"], "") or ""),
        )
    ...
    async def application_status_counts(
        ...
        if employee_id is not None:
            ...
            for row in rows:
                ...
                total += 1
                canonical = canonical_application_status(value)
                if canonical is not None:
                    counts[canonical] += 1
        return [ApplicationStatusCount(status="Всего", count=total)] + [
            ApplicationStatusCount(status=name, count=counts[name])
    ...
    async def _dadata_address(self, address: str) -> dict[str, Any] | None:
        ...
        cache_key = f"geocode:dadata:suggest:v2:{query.casefold()}"
        cached = self._cache_get(cache_key)
        if isinstance(cached, dict):
            return cached or None
        if not self.dadata_api_key:
            raise ClientBaseError("Не настроен API-ключ сервиса адресов")
        try:
            ...
            item = suggestion.get("data", {}) if isinstance(suggestion, dict) else {}
            ...
            resolved = {
                "full_address": str(
                    suggestion.get("unrestricted_value")
            ...
            if exc.response.status_code == 403:
                raise ClientBaseError(
                    "Сервис адресов отклонил запрос: подтвердите почту аккаунта DaData",
                    status_code=403,
                ) from exc
            raise ClientBaseError(
    ...
    async def ensure_application_access(
        self, application_id: int, crm_login: str
    ) -> None:
        """Ensure that a mobile user can access only their assigned application."""
        employee_id = await self._employee_id(crm_login)
        if employee_id is None:
            raise ClientBaseError("Сотрудник не найден", status_code=403)
        payload = await self._request("GET", f"data130/{application_id}")
        attrs = (payload.get("data", {}) or {}).get("attributes", {}) or {}
        assigned_value = str(
            attrs.get(self.fields["application_metrolog_id"], "") or ""
        )
        assigned_ids = set(re.findall(r"\d+", assigned_value))
        if str(employee_id) not in assigned_ids:
            raise ClientBaseError("Заявка не назначена этому сотруднику", status_code=403)
    ...
    async def _nomenclature(self, application_id: int) -> list[NomenclatureItem]:
        ...
        relation_field_is_configured = bool(relation_field)
        metadata: dict = {}
        ...
        if relation_field:
            try:
                ...
                payload = {"data": []}
            rows = payload.get("data", []) or []
        if not rows and not relation_field_is_configured:
            ...
            # instead of downloading the entire table. Never probe when the
            # relation is explicitly configured: another numeric field can
            # accidentally contain the application id and return unrelated
            # nomenclature rows.
